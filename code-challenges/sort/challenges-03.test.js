...
const sortNumbersByLength = (arr) => {
  arr.sort((left, right) => {
    if (left.toString().length < right.toString().length) {return -1;}
    if (left.toString().length > right.toString().length) {return 1;}
    return 0;
  })
  return arr;
...
const sortMeetingsByDay = (arr) => {
  ...
  arr.sort((left, right) => {
    let leftDay = left.dayOfWeek.toLowerCase();
    let rightDay = right.dayOfWeek.toLowerCase();
    if (dayOrder[leftDay] < dayOrder[rightDay]) {return -1;}
    if (dayOrder[leftDay] > dayOrder[rightDay]) {return 1;}
    return 0;
  })
...
const sortSchedule = (arr) => {
  // const meetings = sortMeetingsByDay(arr);
