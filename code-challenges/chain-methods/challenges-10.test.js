...
const createServer = () => {
  ...
  app.get('/hello', (req, res) => {
    res.send('Hi, I\'m Marlene.');
  });

  app.get('/aboutme', (req, res) => {
    res.send('I\'m a student at Code Fellows.');
  });
  app.get('/favoritefoods', (req, res) => {
    const favFoods = ['pizza', 'fruit', 'yogurt', 'pasta'];
    res.send(favFoods);
  });
  app.get('/*', (req, res) => {
    res.sendStatus(404);
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(result => {
    return result.filter(val => val % 5 === 0 && typeof val === 'number').map(num => {
      return Math.pow(2, num);
    })
  })
};
// one liner way for reference
// const divisibleByFiveTwoToThePower = (input) => input.map(result => result.filter(val => val % 5 === 0 && typeof val === 'number').map(num => Math.pow(2,num)));
